...
class HMACTests: XCTestCase {
    ...
    func test_RFC2202_Hmac_SHA1_TestCase1() {
        ...
        let data = "Hi There"
        let hmacSHA1 = base16Encode(HMAC(algorithm: .SHA1, key: key).update(data).final()).lowercaseString
    ...
    func test_RFC4231_Hmac_TestCase1() {
        ...
        XCTAssertEqual(
            base16Encode(HMAC(algorithm: .SHA224, key: key).update(data).final()).lowercaseString,
            "896fb1128abbdf196832107cd49df33f" +
            "47b4b1169912ba4f53684b22")
        XCTAssertEqual(
            base16Encode(HMAC(algorithm: .SHA256, key: key).update(data).final()).lowercaseString,
            "b0344c61d8db38535ca8afceaf0bf12b" +
            "881dc200c9833da726e9376c2e32cff7")
        XCTAssertEqual(
            base16Encode(HMAC(algorithm: .SHA384, key: key).update(data).final()).lowercaseString,
            "afd03944d84895626b0825f4ab46907f" +
                "15f9dadbe4101ec682aa034c7cebc59c" +
            "faea9ea9076ede7f4af152e8b2fa9cb6")
        XCTAssertEqual(
            base16Encode(HMAC(algorithm: .SHA512, key: key).update(data).final()).lowercaseString,
    ...
    func test_RFC4231_Hmac_TestCase5() {
        ...
        let data = base16Decode(
            ...
            "74696f6e")!
        XCTAssertEqual(
            prefix(base16Encode(HMAC(algorithm: .SHA224, key: key).update(data).final()), 32).lowercaseString,
            "0e2aea68a90c8d37c988bcdb9fca6fa8")
        XCTAssertEqual(
            prefix(base16Encode(HMAC(algorithm: .SHA256, key: key).update(data).final()), 32).lowercaseString,
            "a3b6167473100ee06e0c796c2955552b")
        XCTAssertEqual(
            prefix(base16Encode(HMAC(algorithm: .SHA384, key: key).update(data).final()), 32).lowercaseString,
            "3abf34c3503b2a23a46efc619baef897")
        XCTAssertEqual(
            prefix(base16Encode(HMAC(algorithm: .SHA512, key: key).update(data).final()), 32).lowercaseString,
